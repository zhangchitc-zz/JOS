...
在E100处理接受的时候，和前面处理发送几乎是一样的，只是RU处理的是RFD而CU处理的是CB，而且CU可能有多个命令，每个命令对应的CB结构是不一样的，发送对应的CB就是TCB。
...
\begin{enumerate}
\item{首先要记录的是当前RFA环状结构中哪些RFD是等待被用户接收的，哪些是可以被RU重新利用来接受数据的}
